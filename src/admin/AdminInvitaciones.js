...
export default function AdminInvitaciones() {
  ...
  const [orden, setOrden] = useState({ campo: 'reserva', asc: false });
  const { selectedCommunity } = useCommunity();

  useEffect(() => {
    setLoading(true);
    const communityId = getCommunityId(selectedCommunity);
    fetchInvitaciones(communityId).then(res => {
      const data = Array.isArray(res.data) ? res.data : res.data.results || [];
      setInvitaciones(data);
      setLoading(false);
    });
    // Carga usuarios para el filtro
  ...
  // Filtro y ordenación
  const invitacionesFiltradas = invitaciones
    .filter(i =>
      !usuarioFiltro ||
      String(i.reserva?.user?.id) === String(usuarioFiltro)
    )
    .sort((a, b) => {
      ...
      if (campo === 'reserva') {
        ...
        va = a.reserva?.user?.nombre || '';
        vb = b.reserva?.user?.nombre || '';
        ...
        >
          ...
          <div className="table-responsive">
            <table className="table table-striped table-sm align-middle">
              <thead>
                <tr>
                  <th style={{ minWidth: 160, cursor: 'pointer' }} onClick={() => handleOrdenar('reserva')}>Reserva</th>
                  <th style={{ minWidth: 120, cursor: 'pointer' }} onClick={() => handleOrdenar('user')}>Usuario</th>
                  <th style={{ minWidth: 120 }}>Email</th>
                  <th style={{ minWidth: 100, cursor: 'pointer' }} onClick={() => handleOrdenar('estado')}>Estado</th>
              ...
              <tbody>
                {invitacionesFiltradas.map(i => (
                  <tr key={i.id}>
                    ...
                    <td className="text-break" style={{ verticalAlign: 'middle' }}>
                      {i.reserva?.user?.nombre} ({i.reserva?.user?.email})
                    </td>
